...
public struct BXMultiValuePicker : NSViewRepresentable
{
	...
	func identifier(for items:[BXMenuItemSpec]) -> String
	{
		return items.reduce("")
		{
			switch $1
			{
				case .action(_,let title,_,_,_): return $0 + title + "\n"
	...
	private func setColors(of popup:NSPopUpButton)
    {
		...
		cell.fillColor = NSColor(bxColorTheme.fillColor(for:colorScheme, isEnabled:popup.isEnabled))
		cell.strokeColor = NSColor(bxColorTheme.strokeColor(for:colorScheme, isEnabled:popup.isEnabled, enhanceBy: colorScheme == .dark ? 1.0 : 1.0))
...
//		super.drawBorderAndBackground(withFrame:cellFrame, in:controlView)
		...
		if self.controlSize == .regular
		{
            ...
            else if #available(macOS 11,*) // Big Sur requires moving up by an additional 1pt
            {
